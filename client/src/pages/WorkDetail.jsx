...
export default function WorkDetail() {
  ...
  return <>
    <div className="breadcrumb-wrapper bg-cover" style={{ backgroundImage: "url('/assets/img/inner-page/bread-line.png')" }}>
      ...
      <div className="container"><div className="page-heading mb-0"><div className="breadcrumb-sub-title"><h1 className="text-white rr_title_anim"><span>{story.title || project.client}</span></h1></div><div className="breadcrumb-items"><ul><li>{project.tags.join(' · ')}</li><li>{project.sector} | {project.year}</li></ul><h2 className="title wa_title_spilt_1">{project.client}</h2></div></div></div>
    </div>
    <section className="project-details-section fix section-padding">
      <div className="container"><div className="details-thumbs fix"><img src={photos.hero || project.image} alt={project.client} /></div></div>
      <div className="container container-1680"><div className="project-details-wrapper"><div className="project-details-top-item">
        ...
        <div className="row g-4">{project.stats.map((stat, i) => <div className="col-md-4" key={i}><div className="details-box"><h3>{stat.value}</h3><p>{stat.label}</p></div></div>)}</div>
        <div className="slider-button d-flex align-items-center justify-content-between">{previous ? <Link to={`/work/${previous.slug}`}>← {previous.client}</Link> : <span />}<Link to="/work">All projects</Link>{next ? <Link to={`/work/${next.slug}`}>{next.client} →</Link> : <Link to="/contact">Get in touch →</Link>}</div>
      </div></div></div>
    </section>
  </>
}
